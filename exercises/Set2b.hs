...
import Distribution.Simple.Utils (xargs, safeTail)
...
countdown n = "Ready! " ++ counter n
        where counter n 
                | n == 0 = "Liftoff!"
                |otherwise = show n ++ "... " ++ counter (n-1)
...
smallestDivisor n = f n 2 
    where f n k = if mod n k == 0 then k else f n (k+1)
